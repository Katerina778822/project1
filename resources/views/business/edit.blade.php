<x-app-layout>
    ...
    <div class="space-y-10 divide-y divide-gray-900/10 justify-center flex ">
        <div class="grid grid-cols-1 gap-x-8 gap-y-8 max-w-xs mb-4">
            ...
            <form class="bg-gray-200 shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl md:col-span-2 items-center justify-center" method="post" action="{{ route('business.update', ['business' => $item->id]) }}">
                ...
                <div class="flex items-center justify-center border-t border-gray-900/10 px-4 py-4 sm:px-8">
                    <div>
                        ...
                        <!-- Type -->
                        <div>
                            <x-label for="type" :value="__('Имя')" />

                            <x-input id="type" class="block mt-1 w-full" type="text" name="type" value="{{$item->type}}" required autofocus />
                        </div>
